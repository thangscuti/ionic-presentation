...
const Tab1: React.FC = () => {
  let session
  useEffect(() => {
    const uri = Capacitor.convertFileSrc('/board_preview');
    console.log({uri:uri});
    // alert(uri)
    WebServer.start(8441).then((res) => {
      console.log({res});
    }).catch((error) => {
      console.log({error});
    })
    ...
    // if (isPlatform('web')) {
      ...
      if(navigator.presentation){
        navigator.presentation.onavailablechange=function(dict: { available: any; }){
          if(dict.available){
           session = navigator.presentation.requestSession(uri) // deeplink
          //  ('file:///var/containers/Bundle/Application/FB4AF131-24A7-42FF-A2F3-82A2D694A6ED/App.app/www/presentation/display.html#E6AED');
            // session.onmessage((msg: any) => {
            //   console.log(`presentation::${msg}`)
            //   alert('presentation:onmessage')
            // })
  ...
  const onClickSendMessage = () => {
    if(session){
      // session.postMessage(JSON.stringify({message:'a san', lang:'vi'}));
    }
  }
  return (
    <IonPage>
